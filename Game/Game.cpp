...
void Game::play() {
	...
	while (endingLoop != true) { //Loops for calling lies.
		for (unsigned int i = 0; i < players.size(); ++i) { //For loops for score and round
			...
			if (guessedDices == 0) { 
				guessedDices = player.guessNumberOfDice(a, b);
				guessedDiceValue = player.guessDiceValue(guessedDices);
				checkLiar = liar(fullDiceVector, guessedDiceValue, guessedDices);
				//If like is true 
				if (checkLiar == true) {
			...
			//Guess, check side value, call liar and ends the game
				...
				guessedDices = player.increaseQuantityGuesses(guessedDices, b, guessedDiceValue);
				guessedDiceValue = player.guessDiceValue(guessedDices);
				checkLiar = liar(fullDiceVector, guessedDiceValue, guessedDices);
				if (checkLiar == true) {
					endingLoop = finalOutputEndLoop(endingLoop, a, fullDiceVector);
